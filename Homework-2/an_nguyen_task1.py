...
def count_occurrences(chunk: List[str], candidate: List) -> tuple:
    ...
    for basket in chunk:
        for c in candidate:
            for item_set in c[1]:
                if item_set in basket or all([i in basket for i in item_set]):
...
def main():
    ...
    case, min_count, input_file, output_file = sys.argv[1:]
    min_count = int(min_count)

    spark = SparkSession.builder.master("local[*]").appName("HW2").getOrCreate()
    text = spark.sparkContext.textFile(input_file)
    head = text.first()
    data = text.filter(lambda row: row != head)

    if case == "1":
        baskets = data.map(lambda line: (line.split(',')[0], line.split(',')[1]))
    elif case == "2":
        baskets = data.map(lambda line: (line.split(',')[1], line.split(',')[0]))
    else:
        print(f"Invalid case number {case}")
        return
    baskets = baskets.groupByKey().map(lambda x: list(x[1]))
